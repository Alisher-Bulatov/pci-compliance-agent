...
export default function App() {
  ...
  function handleLine(obj: EventLine) {
    ...
    // Render stages in the left log
    if ("stage" in obj && typeof obj.stage !== "undefined") {
      const label = (obj as any).label ?? String((obj as any).stage);
      setSystemLog((l) => [...l, String(label)]);
      return;
    }
    if ("label" in obj && !("type" in obj)) {
    ...
    if ((obj as any).type === "info") {
      setSystemLog((l) => [...l, `ℹ️ ${String((obj as any).message || "")}`]);
      return;
    }
    if ((obj as any).type === "error") {
      setSystemLog((l) => [...l, `❌ ${String((obj as any).message || "")}`]);
      return;
    }
    ...
    // Fallback: dump unknown lines to system log
    ...
    <div style={{ minHeight: "100vh", background: "#0f172a", color: "#e5e7eb" }}>
      <div style={{ maxWidth: 980, margin: "0 auto", padding: "16px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, alignItems: "center" }}>
          <input
            ref={inputRef}
            placeholder="Ask something… e.g., “Is 3.2.1 focused on storage limitations or transmission security?”"
            value={query}
